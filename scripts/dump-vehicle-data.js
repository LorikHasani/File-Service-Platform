...
const API = process.env.API_URL || 'https://localhost:53006';
...
async function main() {
  ...
  for (const carName of Object.keys(tree)) {
    for (const modelName of Object.keys(tree[carName])) {
      ...
      for (const gen of gens) {
        tree[carName][modelName].generations[gen.name] = { _genId: gen.id, engines: {} };
      }
    ...
    for (const modelName of Object.keys(tree[carName])) {
      for (const genName of Object.keys(tree[carName][modelName].generations)) {
        ...
        for (const eng of engines) {
          tree[carName][modelName].generations[genName].engines[eng.name] = { _engId: eng.id, ecus: [] };
        }
  ...
  for (const carName of Object.keys(tree)) {
    for (const modelName of Object.keys(tree[carName])) {
      for (const genName of Object.keys(tree[carName][modelName].generations)) {
        for (const engName of Object.keys(tree[carName][modelName].generations[genName].engines)) {
          const engId = tree[carName][modelName].generations[genName].engines[engName]._engId;
          const ecus = await fetchJson(`/api/ecus/byengine/${engId}`);
          tree[carName][modelName].generations[genName].engines[engName].ecus = ecus.map(e => e.name);
  ...
  const outPath = path.join(process.cwd(), 'public', 'vehicle-data.json');
