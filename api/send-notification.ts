            ...
            <tr>
              <td style="vertical-align:middle;">
                <p style="margin:0 0 4px;font-size:13px;color:#333;">
                  <strong style="color:#2563eb;">${BRAND_NAME}</strong>
                </p>
...
// NOTE: the credit_purchase email type was removed from this endpoint.
// Purchase confirmations are sent exclusively by the Stripe webhook
// (api/stripe-webhook.ts) after signature verification — exposing them here
// let any logged-in user trigger fake "purchase confirmed" emails with
// arbitrary amounts to arbitrary clients.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!type || !jobId || typeof jobId !== 'string') {
      return res.status(400).json({ error: 'Missing type or jobId' });
    }

    // Caller role, used to authorize each notification type below
    ...
    if (type === 'file_delivered') {
      ...
      // Only admins deliver files; anyone else could send fake "delivered"
      // emails to arbitrary clients.
      if (!isAdmin) {
      ...
      // Only the job's owner (or an admin) may trigger this.
      if (!isAdmin && job.client_id !== user.id) {
